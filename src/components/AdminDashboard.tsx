...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ items, onSave, onLogout }) => {
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-6 border-b border-zinc-800">
          <div>
            <h1 className="text-3xl font-black tracking-tight text-white">
              Staff <span className="text-amber-500">Admin Dashboard</span>
            </h1>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl transition-all flex items-center gap-2 shadow-lg shadow-emerald-900/20"
            >
            ...
            <button
              onClick={onLogout}
              className="px-5 py-2.5 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 text-zinc-300 hover:text-white font-bold rounded-xl transition-all flex items-center gap-2"
            >
        ...
        <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl p-6 space-y-6">
          ...
          <form onSubmit={handleAddNewItem} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="md:col-span-2 lg:col-span-3 pt-2">
              <button
                type="submit"
                className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-black font-bold rounded-xl text-sm transition-all flex items-center justify-center gap-2"
              >
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            <span className="text-xs bg-zinc-800 text-zinc-400 px-2.5 py-1 rounded-full font-semibold">
          ...
          <div className="grid grid-cols-1 gap-4">
            {localItems.map((item) => (
              <div
                key={item.id}
                className="bg-zinc-900/30 border border-zinc-800/60 rounded-xl p-4 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 hover:border-zinc-700 transition-colors"
              >
                {/* Left: Item Info */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full lg:w-1/2">
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-1.5 bg-zinc-950 border border-zinc-800 rounded-lg text-white text-sm focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  <div className="space-y-1">
        ...
        <div className="sticky bottom-6 bg-zinc-900/90 backdrop-blur-md border border-zinc-800 p-4 rounded-2xl flex items-center justify-between shadow-2xl">
          <span className="text-sm text-zinc-400">
            You have unsaved changes in your local list.
          </span>
          <button
            onClick={handleSaveChanges}
